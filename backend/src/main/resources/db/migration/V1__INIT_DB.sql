...
COMMENT
ON TABLE oauth_user IS 'Базовая таблица пользователей системы';
COMMENT
ON TABLE user_info IS 'Дополнительная информация о пользователях';
COMMENT
ON TABLE project IS 'Проекты IT планировщика';
COMMENT
ON TABLE employee IS 'Сотрудники, привязанные к проектам';
COMMENT
ON TABLE project_repository IS 'Репозитории файлов проектов';
COMMENT
ON TABLE project_repository_file IS 'Файлы в репозиториях проектов';
COMMENT
ON TABLE task_info IS 'Основная информация о задачах';
COMMENT
ON TABLE task_details IS 'Детальная информация о задачах';
